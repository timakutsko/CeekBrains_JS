...
const btnsColl = document.querySelectorAll('button');
btnsColl.forEach(btn => {
    ...
    product.forEach(prd => {
        let html = `
            <div class="product">    
                ...
                <div>${prd.price}</div>
                <a href="https://example.com/producs/${prd.id}">Подробнее</a>
            </div>
        `;
        prodDiv.insertAdjacentHTML('beforeend', html);
    });
}
